...
const cleanLib = lib => lib.substring(lib.indexOf("'") + 1, lib.lastIndexOf("'"));

const runFunction = i => {
  const split = i.text.split('.');
  const lib = split[0];
  let func = split[split.length - 1];
  console.log('Gutenbuild Running: ', i.text);

  if (lib.startsWith('data')) {
    if (func.startsWith('select')) {
      debugger;
      wp['data']['select'](cleanLib(i.text))[func]();
    } else {
      wp['data']['dispatch'](cleanLib(i.text))[func]();
    }
  } else {
    console.log('Response: ', wp[lib][func]());
  }
};
...
/******/ !function() {
/******/ 	__webpack_require__.h = function() { return "ee88f65cace5801d7f38"; }
/******/ }();
/******/ 
/******/ }
);
//# sourceMappingURL=index.5a8bdaa5b6ec31bd9db6.hot-update.js.map