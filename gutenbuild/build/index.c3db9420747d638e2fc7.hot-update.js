...
function DataTabProvider(_ref) {
  ...
  for (let k = 0; k < storeNames.length; k++) {
    ...
    allData = allData.concat(flatten({
      [`select('${store}')`]: wp.data.select(store)
    }));
  ...
  for (let k = 0; k < storeNames.length; k++) {
    const store = storeNames[k];
    allData = allData.concat(flatten({
      [`dispatch('${store}')`]: wp.data.dispatch(store)
    }));
  }
...
/******/ !function() {
/******/ 	__webpack_require__.h = function() { return "323cc35805d67724189c"; }
/******/ }();
/******/ 
/******/ }
);
//# sourceMappingURL=index.c3db9420747d638e2fc7.hot-update.js.map